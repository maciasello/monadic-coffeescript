...
(function() {
  var Access, Arr, Assign, Base, Block, Call, Class, Code, CodeFragment, Comment, Existence, Expansion, Extends, For, HEXNUM, IDENTIFIER, IS_REGEX, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, NEGATE, NO, NUMBER, Obj, Op, Param, Parens, RESERVED, Range, Return, SIMPLENUM, STRICT_PROSCRIBED, Scope, Slice, Splat, Switch, TAB, THIS, Throw, Try, UTILITIES, Value, While, YES, addLocationDataFn, compact, createBind, createBindCall, createCall, createLambdaAndBind, createThen, del, ends, extend, flatten, fragmentsToText, isComplexOrAssignable, isLiteralArguments, isLiteralThis, locationDataToString, merge, multident, parseNum, ref1, ref2, some, starts, throwSyntaxError, unfoldSoak, utility,
  ...
  createCall = function(monad, args, method) {
    var u;
    u = new Value(monad, [new Access(new Literal(method))]);
    return new Call(u, args);
  };

  createBindCall = function(monad, monadicValue, f) {
    return createCall(monad, [monadicValue, f], 'bind');
  };

  createLambdaAndBind = function(monad, params, val, funcGlyph, block) {
    var callback;
    params = params != null ? params : [];
    callback = new Code(params, Block.wrap([block]), funcGlyph);
    return createBindCall(monad, val, callback);
  };

  createBind = function(monad, params, val, funcGlyph, block) {
    return createLambdaAndBind(monad, params, val, funcGlyph, block);
  };

  createThen = function(monad, val, block) {
    return createLambdaAndBind(monad, null, val, 'boundfunc', block);
  };

  exports.createMonadic = function(monad, lines) {
    var currentBlock, j, line, ref3;
    if (lines[lines.length - 1].type !== 'pass') {
      throw new Error("last line is a bind");
    }
    currentBlock = lines[lines.length - 1].val;
    ref3 = lines.slice(0, -1);
  ...
  exports.createBinderLine = function(params, val, funcGlyph) {
    return {
      type: 'bind',
      params: params,
      val: val,
  ...
  exports.createExpressionLine = function(val) {
    return {
      type: 'pass',
      val: val
    };
